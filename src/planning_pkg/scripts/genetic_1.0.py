...
# 定义待优化的参数范围
PARAMETER_RANGES = {
    "smoothing_window_size": (1, 5),
    "max_distance": (1.0, 5.0),
    "speed_range": (1.0, 5.0)
}
...
def evaluate_individual(individual):
    """评估一个个体的适应度值"""
    # rospy.loginfo("Evaluating individual with parameters: %s", individual)

    # rospy.loginfo("Resetting car position...")
    reset_car_position.publish_initial_pose()
    # rospy.loginfo("Waiting for car position to reset...")
    # rospy.sleep(5)  # 等待足够的时间以确保位置重置成功
    # rospy.loginfo("Car position reset complete.")
    ...
    rospy.loginfo("2. Starting FollowTheGap")
    ftg = FollowTheGap(smoothing_window_size, max_distance, 0.5, (min_speed, max_speed))
    ...
    # 7. 返回适应度值
    # rospy.loginfo("Fitness calculated: %f", fitness)
    return fitness
...
def genetic_algorithm(population_size, generations):
    ...
    for generation in range(generations):
        ...
        for idx, individual in enumerate(population):
            rospy.loginfo(f"1. Evaluating individual {idx + 1} in generation {generation + 1}")
...
if __name__ == '__main__':
    ...
    best_individual = genetic_algorithm(population_size=2, generations=1)
