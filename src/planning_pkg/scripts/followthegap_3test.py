#!/usr/bin/env python3
# for the maxmum distance best point
import rospy 
import numpy as np
...
class followthegap(object):
    ...
    def preprocess_lidar(self, ranges):
        ...
        proc_ranges = np.convolve(proc_ranges, np.ones(5) / 5, mode = 'same')

        proc_ranges = np.where(proc_ranges > 5, 5, proc_ranges)
    ...
    def find_best_point(self, start_i, end_i, ranges):
        ...
        max_distance = ranges[start_i]
        candidate_index = [start_i]
        ...
        for i in range(start_i + 1, end_i + 1):
            if ranges[i] > max_distance:
                ...
                candidate_index = [i]
                best_point_index = i
            elif ranges[i] == max_distance:
                candidate_index.append(i)

        if len(candidate_index) > 1:
            middle_index = start_i + (end_i - start_i) // 2
            best_point_index = min(candidate_index, key=lambda x: abs(x - middle_index))
    ...
    def lidar_callback(self, data):
        ...
        bubble_radius = 0.6 # Bubble radius in meters 
        ...
        speed = min(4, max(1.0, distance / 0.5))
        ...
        if abs(drive_msg.drive.steering_angle) > 45.0 / 180.0 * np.pi:
