...
class AStarPlanner:
    ...
    def planning(self, sx, sy, gx, gy):
        ...
        while 1:
            ...
            c_id = min(open_set, key=lambda i: open_set[i].cost + self.calc_heuristic(goal_node, open_set[i]))
...
class PIDController:
    ...
    def compute_control(self):
        ...
        # 从最近路径点开始，寻找一个与最近路径点距离大于等于前视距离的路径点
        # for i in range(target_idx, len(self.path)):
        #     dist = self.get_distance(self.path[target_idx], self.path[i])
        #     if dist >= LOOK_AHEAD_DIS:
        #         lookahead_idx = i
        #         break
        # else:
        #     lookahead_idx = len(self.path) - 1  # 如果没有找到，选择最后一个路径点
        ...
        if abs(ackermann_cmd.drive.steering_angle) > 20.0 / 180.0 * np.pi:
            ...
            ackermann_cmd.drive.speed = 1.5
        else:
            ackermann_cmd.drive.speed = 2.5
